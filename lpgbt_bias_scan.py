...
def i2cmaster_write(system, reg_addr, data):
    ...
    while (success == 0):
        ...
        if (status >> 6) & 0x1:
            ...
            print(
                Colors.RED + "ERROR: I2C master port finds that the SDA line is pulled low 0 before initiating a transaction. Indicates a problem with the I2C bus." + Colors.ENDC)
            rw_terminate()
    ...
    print(
        "Successful I2C write to slave register: " + reg_addr_string + ", data: " + data_string + " (" + '{0:08b}'.format(
            data) + ")")
...
def i2cmaster_read(system, reg_addr):
    ...
    data_string = "0x%02X" % (data)
    print(
        "Successful read from slave register: " + reg_addr_string + ", data: " + data_string + " (" + '{0:08b}'.format(
            data) + ")")
    return data
...
def main(system, boss, channel, enable, reg_list, data_list, upper, lower):
    if not boss:
        print(
    ...
    lower_num = int(lower, 16)  # convert string to integer
    upper_num = int(upper, 16)

    rssi_array, data_array = [], []
    ...
    for i in range(lower_num, upper_num):
        ...
        data_array.append(hex(i))

    print(len(data_array))
    print(len(rssi_array))
    fig, ax = plt.subplots()
    ax.set_xlabel('Hex')
    ax.set_ylabel('RSSI')
...
if __name__ == '__main__':
    ...
    parser.add_argument("-r", "--reg", action="store", nargs='+', dest="reg",
                        help="reg = list of registers to read/write; only use with type: reg")
    parser.add_argument("-c", "--channel", action="store", dest="channel", nargs='+',
                        help="channel = TX1, TX2, TX3, TX4; only use with type: name")
    parser.add_argument("-e", "--enable", action="store", dest="enable",
                        help="enable = 0 or 1; only use with type: name")
    parser.add_argument("-n", "--name", action="store", dest="name", nargs='+',
                        help="name = biascur_reg, modcur_reg, empamp_reg; only use with type: name")
    parser.add_argument("-d", "--data", action="store", nargs='+', dest="data",
                        help="data = list of data values to write")
    parser.add_argument("-ul", "--upper_limit", action="store", dest="upper_limit", help="upper limit")
    parser.add_argument("-ll", "--lower_limit", action="store", dest="lower_limit", help="lower limit")
    ...
    try:
        main(args.system, boss, args.channel, args.enable, reg_list, data_list, args.upper_limit, args.lower_limit)
